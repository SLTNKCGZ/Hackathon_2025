...
def create_lesson(lesson:LessonRequest,user:user_dependency,db:db_dependency):
    if user is None:
...
def update_lesson(db:db_dependency,user:user_dependency,lesson:LessonRequest,q_id:int):
    if user is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Not authenticated")
...
def update_lesson(db: db_dependency, user: user_dependency, lesson: LessonRequest, n_id: int):
    if user is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not authenticated")
...
def delete(db:db_dependency,user:user_dependency,q_id:int):
    if user is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Not authenticated")
    db_user=db.query(User).filter(User.id == user.get("id")).first()
    if db_user is None:
